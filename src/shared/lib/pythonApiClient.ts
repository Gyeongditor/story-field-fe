...
/**
 * 파이썬 백엔드 API 전용 Axios 인스턴스
 * - 포트 8000 사용
 * - 동화 생성 등 AI 관련 API 전용
 */

const resolvePythonApiUrl = (): string => {
  try {
    ...
    if (isDevice && platform === 'ios') {
      return "http://192.168.200.196:8000";
    }
    ...
    if (platform === 'android') {
      if (isDevice) {
        ...
        return "http://10.0.2.2:8000"; // Android 에뮬레이터 전용
      }
    }
...
export const pythonApiClient: AxiosInstance = axios.create({
  ...
  timeout: 30000, // AI 생성은 시간이 오래 걸릴 수 있으므로 30초로 설정
  headers: {
...
// 요청 인터셉터: 인증 토큰 자동 추가 (스프링 백엔드와 동일한 토큰 사용)
        ...
        if (error.config && !error.config._retry) {
          ...
          try {
            const refreshClient = axios.create({
              baseURL: apiClient.defaults.baseURL, // 스프링 백엔드 URL 사용
              timeout: 10000,
              withCredentials: true,
            });
            ...
            if (newAccessToken) {
              ...
              return pythonApiClient(error.config); // 원래 요청 재시도
            }
          } catch (refreshError) {
            // 갱신 실패 시 로그아웃 처리
            try {
